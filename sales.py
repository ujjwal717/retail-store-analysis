...
db_link = psycopg2.connect(
    "dbname = <your database name>  user= <your database username> "
    "password = <your database password> host = <your host> port = <your port number>"
)


def category_percentage():
    df_beauty = pd.read_sql(
        """SELECT COUNT(product_category) AS order_count, 
        COUNT(product_category)*100/1000 AS percentage, 
        product_category FROM sales_data GROUP BY product_category HAVING product_category = 'Beauty' 
    ...
    df_electronic = pd.read_sql(
        """SELECT COUNT(product_category)AS order_count,
        COUNT(product_category)*100/1000 AS percentage, 
        product_category FROM sales_data GROUP BY product_category HAVING product_category = 'Electronics' 
        ORDER BY product_category""",
        db_link,
    )
    df_clothes = pd.read_sql(
        """SELECT COUNT(product_category) AS order_count,
        COUNT(product_category)*100/1000 AS percentage, 
        product_category FROM sales_data GROUP BY product_category HAVING product_category = 'Clothing' 
        ORDER BY product_category""",
        db_link,
    )
    ...
    df_categories["angle"] = (
        df_categories["percentage"] / df_categories["percentage"].sum() * 2 * pi
    )
    df_categories["color"] = Dark2[len(df_categories["percentage"])]

    p_percentage = figure(
        height=350,
        title="RETAIL SALES PERCENTAGE ACCORDING TO CATEGORIES",
        toolbar_location=None,
        tools="hover",
        tooltips="@product_category: @percentage",
        x_range=(-0.5, 1.0),
    )
    ...
    p_percentage.wedge(
        ...
        line_color="white",
        fill_color="color",
        legend_field="product_category",
        source=df_categories,
    )
...
def avg_amount():
    df_avg_beauty = pd.read_sql(
        ...
        avg(total_amount) AS average_amount FROM sales_data GROUP BY product_category 
        HAVING product_category = 'Beauty' """,
        db_link,
    )
    df_avg_electronics = pd.read_sql(
        """SELECT product_category, 
        avg(total_amount) AS average_amount FROM sales_data 
    ...
    source = ColumnDataSource(
        data=dict(category=category, average_amount=average_amount)
    )
    p_avg_amount = figure(
        x_range=category,
        height=500,
        toolbar_location=None,
        tools=[TOOLTIPS],
        title="AVERAGE OF TOTAL AMOUNT SPENT BY CUSTOMERS IN DIFFERENT CATEGORIES",
    )
    p_avg_amount.vbar(
        x="category",
        top="average_amount",
        ...
        line_color="white",
        fill_color=factor_cmap("category", palette=Bright6, factors=category),
    )
...
def monthly_sales():
    df_sales = pd.read_sql(
        ...
        COUNT(transaction_id) OVER(PARTITION BY date) AS sales_per_day 
    ...
    fig_0 = px.line(
        ...
        title="REATAIL SALES INSIGHT DIVIDED ACCORDING TO MONTHS AND DAYS",
...
def gender_quantity():
    df_male_quantity = pd.read_sql(
        """SELECT customer_id, 
        gender, date, sum(quantity) OVER(PARTITION BY date ) AS purchase_quantity 
...
def gender_product_category():
    df_male_electronics = pd.read_sql(
        """SELECT SUM(quantity) AS quantity_ordered, 
        product_category,gender FROM sales_data WHERE gender = 'Male' 
...
def age_quantity():
    df_age_quantity = pd.read_sql(
        """SELECT distinct age, SUM(quantity) OVER(PARTITION BY age) AS total_quantity 
        FROM sales_data""",
        db_link,
    )
